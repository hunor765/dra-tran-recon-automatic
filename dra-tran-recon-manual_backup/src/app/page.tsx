          ...
          <>
              ...
              <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                <div>
                  ...
                  <p className="text-white/80">
                    {results.summary.backend_total_value >= results.summary.ga4_total_value ? (
                      ...
                      <>GA4 reports <span className="font-bold text-white">{formatCurrency(results.summary.ga4_total_value - results.summary.backend_total_value)}</span> more than backend (possible duplicate tracking).</>
                    )}
                  </p>
                </div>
            ...
            <div className="grid md:grid-cols-4 gap-4 mb-8">
              ...
              <div className="card text-center">
                ...
                {(() => {
                  const diff = results.summary.backend_total_value - results.summary.ga4_total_value;
                  const color = diff > 0 ? "text-destructive" : diff < 0 ? "text-warning" : "text-success";
                  const sign = diff > 0 ? "-" : diff < 0 ? "+" : "";
                  return <p className={`text-2xl font-bold ${color}`}>{sign}{formatCurrency(Math.abs(diff))}</p>;
                })()}
              </div>
              <div className="card text-center">
              ...
              <div className="card mb-8">
                ...
                <div className="h-64 w-full relative">
                  ...
                  <svg className="w-full h-full overflow-visible" preserveAspectRatio="none">
                    ...
                    {[0, 25, 50, 75, 100].map((tick, i) => (
                      <g key={tick}>
                        <line
                          ...
                          strokeDasharray="4 4"
                        />
                        <text x="-10" y={`${100 - tick}%`} fill="#9ca3af" fontSize="10" alignmentBaseline="middle" textAnchor="end">{tick}%</text>
                      </g>
                    ))}

                    {/* Bars for Total Volume (Normalized to max volume) */}
                    {(() => {
                      const maxVol = Math.max(...results.temporal_analysis.map(d => d.backend_total)) || 1;
                      const count = results.temporal_analysis.length;
                      const barWidth = count > 1 ? 80 / count : 10;
                      return results.temporal_analysis.map((d, i) => {
                        const x = count > 1 ? (i / (count - 1)) * 100 : 50;
                        const h = (d.backend_total / maxVol) * 40; // Max height 40%
                        return (
                          <rect
                            key={`bar-${i}`}
                            x={`${count > 1 ? x - 0.5 : 45}%`}
                    ...
                    {results.temporal_analysis.length > 1 && (
                      <path
                        ...
                        transform="scale(1, 1)" // Ensure it scales with SVG viewBox
                      // Note: For percent based coordinates to work in path in some browsers we might need absolute pixels
                      // But let's try pure percent logic by setting viewBox 0 0 100 100 and allow stretch
                      />
                    )}

                    {/* Dots */}
                    {(() => {
                      // Fix for SVG path percentage coordinates issues: use viewBox 0 0 100 100
                      // But effectively we are using percentages in x/y props of rects/text
                      return results.temporal_analysis.map((d, i) => {
                        const x = results.temporal_analysis.length > 1 ? (i / (results.temporal_analysis.length - 1)) * 100 : 50;
                        const y = 100 - d.match_rate;
                        return (
                          <g key={i} className="group">
                            <circle cx={`${x}%`} cy={`${y}%`} r="4" fill="#dd3333" className="stroke-white stroke-2" />
                            {/* Tooltip on Hover */}
                            <g className="opacity-0 group-hover:opacity-100 transition-opacity">
                              <rect x={`${x}%`} y={`${y - 15}%`} width="80" height="35" rx="4" fill="#1f2937" transform="translate(-40, 0)" />
                              <text x={`${x}%`} y={`${y - 10}%`} fill="white" fontSize="10" textAnchor="middle" fontWeight="bold">
                                {d.date}
                              </text>
                              <text x={`${x}%`} y={`${y - 6}%`} fill="#d1d5db" fontSize="9" textAnchor="middle" dy="10">
                                Rate: {d.match_rate}% ({d.matched}/{d.backend_total})
                              </text>
                            </g>
                          </g>
                        );
                  ...
                  <div className="flex justify-between mt-2 text-xs text-muted-foreground">
                    {results.temporal_analysis.filter((_, i) => i % Math.ceil(results.temporal_analysis.length / 5) === 0).map((d, i) => (
                      <span key={i}>{d.date}</span>
                    ))}
                    {/* Always show last date if not included */}
                    {results.temporal_analysis.length > 5 && (
                      <span>{results.temporal_analysis[results.temporal_analysis.length - 1].date}</span>
                    )}
                  </div>
                </div>
              </div>
